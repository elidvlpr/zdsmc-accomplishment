      ...
      function(data) {
        ...
        if (data.length > 0) {
          for (let i = 0; i < data.length; i++) {
            ...
            let formattedDate = logDate.toLocaleDateString("en-US", { year: 'numeric', month: 'long', day: '2-digit' });
            let dayOfWeek = logDate.toLocaleDateString("en-US", { weekday: 'long' });
            
            let isWeekend = (dayOfWeek === "Saturday" || dayOfWeek === "Sunday");
            let daylog = `${formattedDate}`;

            if(!isWeekend){
              var workdetail ="Encoded drugs and medicine in the PhilHealth E-Claims system, transcribed doctor's orders, and processed patient discharge with CF4 generation to XML."
            } else {
              var workdetail = `<span style='color: rgb(248, 48, 35); font-weight: bold;'>${dayOfWeek.toUpperCase()}</span>`;
            }

            let dateCell = $("<td></td>").text(daylog);
            let workCell = $("<td class='text-center'></td>").html(workdetail);
            ...
            let qtyCell = $("<td class='text-center text-white'></td>").text(encodedValue);
